...
class Ucf101(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        if self.cfg.SSL.ENABLE and self.cfg.SSL.MODE == "pretext" and self.cfg.SSL.TRANSFORMS == "image":
            import torchvision.transforms as T
            ssl_crop_size = self.cfg.DATA.TRAIN_CROP_SIZE
            self.ssl_transforms = T.Compose([
                T.RandomResizedCrop(ssl_crop_size, scale=(0.2, 0.8)),
                T.RandomHorizontalFlip(),
                T.RandomApply([T.ColorJitter(0.4,0.4,0.4,hue=(0., 0.))], p=0.8),
                T.RandomGrayscale(p=0.2),
                T.RandomApply([T.GaussianBlur(kernel_size=ssl_crop_size//20*2+1, sigma=(0.1, 2.0))], p=0.5),
    ...
    def _get_seq_frames(self, index):
        ...
        if self.cfg.SSL.ENABLE and self.cfg.SSL.T_SAMPLE_JITTER and self.cfg.SSL.MODE == "pretext":
            sampling_rate = random.sample(self.cfg.SSL.SAMPLE_RATE_INT, 1)[0]
        else:
        ...
        for i_try in range(self._num_retries):
            ...
            if self.cfg.SSL.ENABLE and self.cfg.SSL.MODE == "pretext":
                if self.cfg.SSL.TRANSFORMS == "video":
                    # T H W C -> C T H W.
                    ...
                    frames = transform.color_normalization(
                        frames,
                        np.array(self.cfg.DATA.MEAN, dtype=np.float32),
                        np.array(self.cfg.DATA.STD, dtype=np.float32),
                    )
                    # T C H W -> C T H W
                    frames = frames.permute(1, 0, 2, 3)

                elif self.cfg.SSL.TRANSFORMS == "image":
                    # T H W C -> T C H W.
                    frames = frames.permute(0, 3, 1, 2)
    
                    frames = self.ssl_transforms(frames)
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.SSL.ENABLE and self.cfg.SSL.MODE == "pretext":
            frames_1 = self._get_seq_frames(index)
            frames_2 = self._get_seq_frames(index)
            return [frames_1, frames_2], label, index, {}
        else:
